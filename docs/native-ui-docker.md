...
新设备在“添加后端”页面的“API 密钥”中输入 Session API Key。这是本地测试服务的连接密钥，与模型提供商的 API Key 不同。在运行 Docker 的 Mac 上获取：

```sh
docker exec openhands-native-ui cat /home/openhands/.openhands/agent-canvas/api-key.txt
```
...
容器名为 `openhands-native-ui`，镜像为 `openhands-native-ui:1.14.0`。服务绑定 `0.0.0.0:18800`，并配置 `unless-stopped` 重启策略。该端口映射到入口脚本现有的 `PUBLIC_MODE_PORT=18002`，前端要求会话密钥登录；容器内部的自动登录入口 8000 不向宿主机发布。
...
- Session API Key 与设置加密密钥由入口脚本随机生成，保存到状态卷。局域网前端不会在 HTML 中注入会话密钥，API 继续使用现有的 `X-Session-API-Key` 鉴权。
- 此测试部署在构建与运行时关闭产品遥测。
