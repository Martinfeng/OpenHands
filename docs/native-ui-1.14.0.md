...
- 活动标题流光在浅色模式使用 `#555b65` 底字和 `#080a0d` 光带，深色模式使用 `#a3aab5` 底字和 `#ffffff` 光带。两个模式统一使用 500 字重。光带半宽约 `5.2ch`（贴近 Codex TUI 的 5 字符半宽与 OpenCode 的 `--spread: 5.2ch`）；渐变画布为文字宽度的 3.6 倍，光带进出都出界，线性无限循环。等待空档的中文文案为「正在思考」，比「思考中」更长，扫光更完整。一轮固定 2 秒（与 Codex TUI `sweep_seconds = 2` 相同），短标题看起来慢、长标题看起来快。
