...
- **Mock LLM server** (`tests/e2e/mock-llm/scripts/mock-llm-server.py`): Python HTTP server using openhands-sdk's `TestLLM` to return scripted tool-call + text trajectories. Supports admin API endpoints for dynamic trajectory management:
  ...
  - Background title generation in Agent Server 1.42.1 uses the same LLM as the conversation. The mock recognizes the SDK's exact title-system-prompt prefix (without tools) and returns a fixed title separately, without consuming scripted turns or adding to `/admin/requests`. This prevents nondeterministic races between title generation and agent replies.
  - Trajectory turns may set `delay_seconds` (0–60) to hold a model response for interruption/reconnect tests. The mock uses a threaded HTTP server and serializes script consumption under a lock, so a cancelled request does not block admin requests or the resumed conversation. Each delayed request consumes its own turn before waiting; the next request receives the next turn.
- **Native UI regression**: `conversations/native-ui.spec.ts` runs against Agent Server 1.42.1 and covers approvals/rejection, stop/continue, WebSocket reconnection, math, table-derived charts, theme switching and narrow layouts. Set `OH_CANVAS_SAFE_VITE_PORT` to an unused port if the default static frontend port 3001 is occupied. Confirmation tests use the server's existing `security_analyzer: "none"` setting with confirmation enabled to exercise `AlwaysConfirm` without an extra LLM security-analysis call.
...
- Current `overrides` in `package.json` and the advisories they address (keep this list in sync when adding/removing overrides):
  ...
  - `dompurify: 3.4.14` — keeps Monaco's nested DOMPurify on a patched release for XSS bypasses fixed after 3.4.12, including the `IN_PLACE` hook-removal issue (`<= 3.4.12`) and allow-listed risky-tag bypasses. Mermaid SVG sanitizing imports this same version.
